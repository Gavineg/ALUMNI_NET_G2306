...
- 学生管理：列表、搜索、新增（支持 `is_teacher` 标记）、编辑、删除、重置密码
- 违禁词管理：添加 / 删除
- 地图外观设置：出发点坐标（支持地名地理编码）、学校名称、教师列表（姓名/科目/联系方式/备注，可增删）、节点颜色模式、统一色拾色器

### 教师后台（同 `admin.html` 入口，is_teacher 账号登录）
...
- 上传头像/留言配图（拖拽或点击上传，自动压缩至 300KB 以内）
